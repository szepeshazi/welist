...
abstract class _AuthService with Store {
  ...
  @observable
  FirestoreEntity<we.User> user;

  @observable
  FirestoreEntity<we.PublicProfile> publicProfile;
  ...
  Future<void> fetchUserAccount() async {
    FirestoreEntity<we.User> currentUser;
    FirestoreEntity<we.PublicProfile> currentPublicProfile;
    ...
    if (!userSnapshot.exists) {
      ...
      currentUser = FirestoreEntity<we.User>(newUser, userRef);
    } else {
      currentUser =
          FirestoreEntity<we.User>(j.juicer.decode(userSnapshot.data(), (_) => we.User()), userSnapshot.reference);
    }
    ...
    if (!publicProfileSnapshot.exists) {
      ...
      currentPublicProfile = FirestoreEntity<we.PublicProfile>(newPublicProfile, publicProfileRef);
    } else {
      currentPublicProfile = FirestoreEntity<we.PublicProfile>(
