...
import 'package:welist_common/common.dart' as we;
...
abstract class _AuthService with Store {
  ...
  Future<void> _userUpdate(User update) async {
    if (update == null) {
      ...
      print(
          "Auth userUpdate: ${update.email} logged in, verified: ${update.emailVerified}");
    }
    ...
    if (!_fbAuth.currentUser.emailVerified) {
      ...
      if (verificationStatusTimer == null) {
        ...
        verificationStatusTimer =
            Timer.periodic(verificationStatusPollPeriod, (timer) {
          ...
          print(
              "verificationStatusTimer: ${_fbAuth.currentUser.emailVerified}");
          if (_fbAuth.currentUser.emailVerified) {
            timer.cancel();
            timer = null;
            fetchUserAccount();
  ...
  Future<void> fetchUserAccount() async {
    ...
    DocumentSnapshot userSnapshot = await _fs
        .collection(we.User.collectionName)
        .doc(_fbAuth.currentUser.uid)
        .get();
    if (!userSnapshot.exists) {
      DocumentReference userRef =
          _fs.collection(we.User.collectionName).doc(_fbAuth.currentUser.uid);
      String displayName = _fbAuth.currentUser.displayName ??
          _fbAuth.currentUser.email.split("@").first;
      ...
      currentUser = j.juicer.decode(userSnapshot.data(),
          (_) => we.User()..reference = userSnapshot.reference);
    }

    DocumentSnapshot publicProfileSnapshot = await _fs
        .collection(we.PublicProfile.collectionName)
        .doc(_fbAuth.currentUser.uid)
        .get();
    if (!publicProfileSnapshot.exists) {
      print("creating public profile for ${_fbAuth.currentUser.email}");
      DocumentReference publicProfileRef = _fs
          .collection(we.PublicProfile.collectionName)
          .doc(_fbAuth.currentUser.uid);
      String displayName = _fbAuth.currentUser.displayName ??
          _fbAuth.currentUser.email.split("@").first;
      ...
      currentPublicProfile = j.juicer.decode(
          publicProfileSnapshot.data(),
          (_) =>
              we.PublicProfile()..reference = publicProfileSnapshot.reference);
    }
    ...
    if (now - (lastVerificationEmailSent ?? 0) > verificationEmailDelay) {
      try {
        ...
        if (e.code == 'too-many-requests') {
          print(
              "Too many attempts at sending verification email to ${_fbAuth.currentUser.email}");
        } else {
          rethrow;
        }
      } finally {
